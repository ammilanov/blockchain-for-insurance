    ...
    const cards = Array.isArray(claims) ? claims
      ...
      .map(claim => (
        <ClaimComponent key={claim.uuid} claim={claim}
          onRepair={() => {
            claimProcessingActions.processClaim(claim.contractUuid, claim.uuid, 'R', 0);
          }}
          onRefund={(refundable) => {
            claimProcessingActions.processClaim(claim.contractUuid, claim.uuid, 'F', refundable);
          }}
          onReject={() => {
            claimProcessingActions.processClaim(claim.contractUuid, claim.uuid, 'J', 0);
          }} />
      )) : null;
    const claimsDisplay = (Array.isArray(cards) && cards.length > 0) || cards === null ? cards :
      (
    ...
    return (
      <Loading hidden={!loading} text={intl.formatMessage({ id: 'Loading Claims...' })}>
        ...
        <div className='ibm-columns ibm-cards' style={{ minHeight: '30vh' }}
          data-widget='masonry' data-items='.ibm-col-3-1'>
          {claimsDisplay}
        </div>
      </Loading>
    );
