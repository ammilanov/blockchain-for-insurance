...
class RepairOrdersPage extends React.Component {
  ...
  render() {
      ...
      <div key={index} className='ibm-col-5-1 ibm-col-medium-6-2'>
        <div className='ibm-card ibm-border-gray-50'>
          <div className='ibm-card__content'>
            <h4 className='ibm-bold ibm-h4'><FormattedMessage id='Repair Order' /></h4>
            <div style={{ wordWrap: 'break-word' }}>
              <p>
                <FormattedMessage id='Brand' />: {repairOrder.item.brand} <br />
                <FormattedMessage id='Model' />: {repairOrder.item.model} <br />
                <FormattedMessage id='Description' />: {repairOrder.item.description} <br />
              </p>
              <p>
                <button type='button' className='ibm-btn-sec ibm-btn-small ibm-btn-blue-50'
                  onClick={() => { repairServiceActions.completeRepairOrder(repairOrder.uuid); }}>
    ...
    const orders = (Array.isArray(cards) && cards.length > 0) || cards === null ? cards :
      (
    ...
    return (
      <Loading hidden={!loading} text={intl.formatMessage({ id: 'Loading Repair Orders...' })}>
